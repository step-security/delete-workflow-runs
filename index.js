...
async function validateSubscription() {
  ...
  const upstream = "mattraks/delete-workflow-runs";
...
function filterRunsByDailyRetention(runs, keepMinimumRuns, retainDays) {
  ...
  runs.forEach((run) => {
    ...
    if (!runsByDate[dateKey]) runsByDate[dateKey] = [];
...
async function run() {
  try {
    ...
    if (!repositoryInput) throw new Error('Missing required input: repository (expected "owner/repo")');
    const [repoOwner, repoName] = repositoryInput.split("/");
    if (!repoOwner || !repoName) throw new Error(`Invalid repository: "${repositoryInput}". Use "owner/repo".`);
    ...
    if (deleteWorkflowByStatePattern.toUpperCase() !== "ALL") {
      ...
      filteredWorkflows = filteredWorkflows.filter(({ state }) => states.includes(String(state ?? "").toLowerCase()));
    }
    ...
    for (const workflow of filteredWorkflows) {
      ...
      if (useDailyRetention) {
        ...
        if (runsToRetain.length > 0) core.info(`🔄 Retaining latest ${runsToRetain.length} day(s) of runs`);
      }
